...
import { type BinderOption, color, type DispatchMessageContext, format, Level, type WorkerHandler } from './deps.ts';
...
export class Handler implements WorkerHandler {
  ...
  public constructor(options: BinderOption) {
    ...
    this.options.template = this.options.template ?? '[{{timestamp}}] {{level}}: {{message}} {{args}}';
  ...
  public async receive({ context }: DispatchMessageContext): Promise<void> {
    ...
    switch (context.level) {
      case Level.TRACE: {
        console.info(this.variable(...variables, ['level', `${color.brightCyan(level)}`]));
        break;
      }
      case Level.INFORMATION: {
        console.info(this.variable(...variables, ['level', `${color.brightBlue(level)}`]));
        break;
      }
      case Level.WARNING: {
        console.info(this.variable(...variables, ['level', `${color.brightYellow(level)}`]));
        break;
      }
      case Level.SEVERE: {
        console.info(this.variable(...variables, ['level', `${color.brightRed(level)}`]));
        break;
      }
      default: {
        console.info(this.variable(...variables, ['level', `${color.brightGreen(level)}`]));
      }
    }
  }
